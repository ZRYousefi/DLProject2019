...
class Generator(data.Dataset):
    def __init__(self, root, args, partition='train', dataset='omniglot'):
        ...
        assert (dataset == 'omniglot' or dataset == 'mini_imagenet' or dataset == 'food-101'), 'Incorrect dataset partition'
        ...
        if dataset == 'omniglot':
            ...
            self.loader = food101.Food101(self.root)
            self.data, self.label_encoder = self.loader.load_dataset(self.partition == 'train', self.size)
        else:
